...
###`Mp3Importer` class
Let's start with the Mp3 Importer. Build an `Mp3Importer` class that parses a directory of files and sends the filenames to a song class to create a library of music with artists that are unique. To do this, you'll need three methods: `Mp3Importer#path`, `Mp3Importer#files`, `Mp3Importer#import`
...
This method should take the name that is passed in (remember, it will be a string), and do one of two things. Find the artist instance that has that name or create one if it doesn't exist. Either way, the return value of of the method will be an instance of an artist.

####`Artist#add_song(song)`
This method exists to tell the artist about its songs. Simply take the song instance that is passed in as an argument and store it in a `songs` array with all the other songs that belong to the artist.
